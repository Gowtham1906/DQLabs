# PreToolUse Hook: Block reading/listing files in sensitive folders
...
try {
    ...
    # If we can't parse input, allow the call (fail-open for non-security tools)
    exit 0
}
...
if ($tool_input.command)         { $paths_to_check += [string]$tool_input.command }
if ($tool_input.pattern)         { $paths_to_check += [string]$tool_input.pattern }
if ($tool_input.includePattern)  { $paths_to_check += [string]$tool_input.includePattern }
if ($tool_input.query)           { $paths_to_check += [string]$tool_input.query }
...
foreach ($p in $paths_to_check) {
    ...
    if ($normalized -match '(^|/)\.\.(/|$)') {
        [Console]::Error.WriteLine("BLOCKED: Path traversal ('..') detected in path: $p. This is not allowed for security reasons.")
        exit 2
    }

    # --- Check against all sensitive path patterns ---
    foreach ($pattern in $SENSITIVE_PATTERNS) {
        if ($normalized -match $pattern) {
            [Console]::Error.WriteLine("BLOCKED: Access to sensitive path is not allowed. Matched pattern '$pattern' in path: $p. Sensitive files (environments/, .env, .git/, keys, credentials) must not be accessed.")
            exit 2
        }
    }
}
