...
from dqlabs.app_helper.catalog_helper import ( get_asset_metrics, get_attributes_metrics, 
                                              get_alerts_metrics, get_issues_metrics, 
                                              get_measure_metrics, get_domain_metrics)
...
def check_catalog_schedule(config, channel):
    is_exit = False
    ...
    with connection.cursor() as cursor:
        query_string = f"""
            ...
            where schedules_mapping.level='catalog' 
            and lower(channels.name)='{channel}'
            and channels.is_active is true
        """
        cursor = execute_query(connection, cursor, query_string)
        schedule_exit = fetchall(cursor)
...
def alation_catalog_update(config, channel):
    try:
        ...
        if metrics and isinstance(metrics, dict):
            ...
            measure_details = config.get("measure", [])
            if measure_details:
                measure_details = measure_details[0] if isinstance(measure_details, list) else measure_details
                measure_details = measure_details.get('id')

                token = get_alation_token(channel)
                if token and validate_channel_config_permission(channel, 'import', 'domains'):
                    domains = get_alation_domains(channel, token)
                    channel.update(
                        {"domains": domains if domains else None})
                    clean_alation_domains(config, channel)
                    prepare_insert_alation_domains(config, channel)
                set_alation_domain_measure_metrics(
                    config, channel, token, measure_details)
    except Exception as e:
        log_error(
            f"Alation Catalog Update Failed ", e)
        raise e


def call_api_request(endpoint, method_type, params='', auth_token=''):
    try:
        ...
        log_error(
            f"Alation Connector - Get Response Error", e)
...
def get_alation_token(channel):
    ...
    try:
        ...
        response = call_api_request(api_url, 'post', api_params)
...
def get_alation_datasource(config, channel, metrics, token):
    try:
        ...
        if server_host and database_name:
            api_url = f"{channel_host_url}/integration/v1/datasource/?limit=1000"
            datasources = call_api_request(api_url, 'get', '', token)

            """ 
            This code retrieves all valid data sources from Alation API and filters them based on the user-selected data sources in DQLabs.
            """
            ...
            if datasources and valid_datasources:
                ...
                if datasources:
                    ...
                    for datasource in filter_datasources:
                        get_alation_schema(
                            config, channel, metrics, token, datasource.get('id'))
            # v2 ocf connector api configuration
            api_url = f"{channel_host_url}/integration/v2/datasource/"
            datasources = call_api_request(api_url, 'get', '', token)
            datasources = list(filter(lambda datasource: datasource.get(
                "title") if datasource.get("title") in valid_datasources else '', datasources))
            if datasources:
                for datasource in datasources:
                    ocf_id = datasource.get('id', '') if datasource else ''
                    schema_url = f"{channel_host_url}/integration/v2/schema/?ds_id={ocf_id}"
                    all_ocf_schemas = call_api_request(
                        schema_url, 'get', '', token)
                    ocf_schemas = [item.get('name') for item in all_ocf_schemas]
                    ocf_schemas = [schemas.lower() for schemas in ocf_schemas]
                    schema_database = f"""{database_name}.{schema}"""
                    if schema in ocf_schemas or schema_database.lower() in ocf_schemas:
                        get_alation_schema(
                            config, channel, metrics, token, ocf_id)
...
def get_alation_schema(config, channel, metrics, token, ds_id):
    try:
        credentials = metrics.get('credentials', None)
        properties = metrics.get('properties', None)
        channel_host_url = channel.get('host', '')
        ...
        if schema_name and database_name:
            ...
            schemas = call_api_request(api_url, 'get', '', token)
            if schemas:
                schemas = list(filter(lambda schema: schema.get("name").lower(
                ) == schema_name.lower() or schema.get("name").lower() == database_name.lower(), schemas))
                for schema in schemas:
                    set_alation_table_metrics(
                        config, channel, metrics, token, schema)
    except Exception as e:
        log_error(
...
def set_alation_table_metrics(config, channel, metrics, token, schema):
    try:
        ...
        if schema_id and ds_id:
            ...
            tables = call_api_request(api_url, 'get', '', token)
            alert_count, distinct_count = get_issue_count(config, asset_id)
            if tables and len(tables) > 0:

                # Map Alation Domain to DQLabs
                if validate_channel_config_permission(channel, 'import', 'domains'):
                    map_alation_domain_to_dqlabs(config, channel, tables[0])
                ...
                if validate_channel_config_permission(channel, 'export', 'summary', 'asset'):
                    asset_summary_table = f"""<table style="border-style: inset;">\
                                                ...
                                                <tbody>\
                                                    ...
                                                    <tr>\
                                                        <td>Active Rules</td>\
                                                        <td><strong>{metrics.get('active_measures', ''):,}</strong></td>\
                                                    </tr>\
                                                    <tr>\
                                                        <td>Observed Rules</td>\
                                                        <td><strong>{metrics.get('observed_measures', ''):,}</strong></td>\
                                                    </tr>\
                                                    <tr>\
                                                        <td>Scoring Rules</td>\
                                                        <td><strong>{rounding(metrics.get('scored_measures', '')):,}</strong></td>\
                                                    </tr>\
                                                    <tr>\
                ...
                set_alation_columns_metrics(
                    config, channel, metrics, token, tables[0])
    except Exception as e:
        log_error(
            f"Alation Connector - Get Tables ", e)


def set_alation_columns_metrics(config, channel, metrics, token, table):
    try:
        metrics = get_attributes_metrics(config)
        channel_host_url = channel.get('host', '')
        ...
        if metrics and table_id and schema_id and ds_id:
            ...
            columns = call_api_request(api_url, 'get', '', token)
...
# NEW CODE FOR DOMAIN STARTS FROM HERE
    ...
    try:
            ...
            query_string = f"""
                ...
                and id in (
                    select
                        ...
                        join core.connection as cType on cType.id = asset.connection_id
                        left join core.attribute on attribute.asset_id = asset.id and attribute.is_selected=true
                        left join core.domain_mapping as domain_mapping on domain_mapping.asset_id=asset.id
                        where asset.id='{asset_id}' and cType.is_active = true and asset.is_active = true and asset.is_delete = false
                        group by domain_mapping.domain_id 
                )
            """
            cursor = execute_query(connection, cursor, query_string)
            asset_domain_list = fetchall(cursor)
            filter_domain_list = list(
                map(lambda domain_list: f"""{domain_list.get("id")}""",
                    asset_domain_list)
            )
        ...
        if domains and len(domains) > 0:
            ...
            custom_field = call_api_request(
                custom_field_value_url, 'get', '', token)
            ...
            for domain in domains:
                ...
                if domain_id and glossary_id:
                    if glossary_id in filter_domain_list:
                        ...
                        if validate_channel_config_permission(channel, 'export', 'summary', 'domain'):
                            ...
                            domain_metrices_table = f"""<table style="border-style: inset;">\
                                                        ...
                                                        <tbody>\
                                                            <tr>\
                                                                <td>DQ Score</td>\
                                                                <td><strong>{rounding(metrics.get('score', ''))}%</strong></td>\
                                                            </tr>\
                                                            <tr>\
                                                                <td>Tables</td>\
                                                            ...
                                                            <tr>\
                                                                <td>Alerts</td>\
                                                                <td><strong><a href="{client_origin}/remediate/alerts?domain={glossary_id}&hierarchy=true" target="_blank" rel="noopener noreferrer">{metrics.get('alerts', 0)}</a></strong></td>\
                                                            </tr>\
                                                            <tr>\
                                                                <td>Issues</td>\
                                                                <td><strong><a href="{client_origin}/remediate/issues?domain={glossary_id}&hierarchy=true" target="_blank" rel="noopener noreferrer">{metrics.get('issues', 0)}</a></strong></td>\
                                                            </tr>\
                                                            <tr>\
                                                                <td>Measures</td>\
    ...
    try:
        ...
        if metrics and len(metrics) > 0:
            ...
            if validate_channel_config_permission(channel, 'export', 'measures', 'asset'):
                ...
                asset_has_alets = list(filter(lambda attribute: attribute.get(
                    "level") == 'asset' and attribute.get("asset_alerts", 0) > 0, metrics))
                asset_has_issues = list(filter(lambda attribute: attribute.get(
                    "level") == 'asset' and attribute.get("asset_issues", 0) > 0, metrics))
                ...
                for measure in asset_measures:
                    ...
                    alation_values.append({
                        ...
                        "status": status,
                        "value": measure_value,
                        "url": f"{client_origin}/observe/data/{str(measure.get('asset_id'))}/measures"
                    })
            ...
            if validate_channel_config_permission(channel, 'export', 'measures', 'attribute'):
                for column in columns:
                    ...
                    ) == column.get('name').lower() and attribute.get("attribute_alerts", 0) > 0, metrics))

                    attribute_has_issues = list(filter(lambda attribute: attribute.get("level") == 'attribute' and attribute.get("attribute_name", '').lower(
                    ) == column.get('name').lower() and attribute.get("attribute_issues", 0) > 0, metrics))
...
def get_alation_flags(channel, token):
    ...
    try:
        ...
        flags = call_api_request(api_url, 'get', '', token)
    except Exception as e:
        log_error(
    ...
    try:
        ...
        for flag in flags:
            ...
            if obj_id == id:
                ...
                call_api_request(api_url, 'delete', '', token)
...
def set_alation_domain_measure_metrics(config, channel, token, measure_details):
    try:
        query_string = f"""
            with asset_measures AS (    
            select 
                    distinct on (measure.id)
                        measure.id, measure.technical_name as technical_name, base_measure.name,base_measure.description,base_measure.level,base_measure.category,base_measure.default_active,base_measure.disable_scoring,
                        measure.is_active,measure.allow_score,measure.is_positive, measure.status, measure.invalid_rows, measure.row_count, measure.valid_rows, measure.is_drift_enabled,
                        dimension.name as dimension_name, asset.name as asset, asset.id as asset_id,  attribute.id as attribute_id,
                        attribute.name as attribute,measure.created_date,case when measure.is_auto = true then 'auto' else 'manual' end as threshold_type,
                        case when dimension.id is not null then json_agg(jsonb_build_object('id',dimension.id,'name',dimension.name,'color',dimension.color)) else null end as dimension,
                        connection.name as connection_name,connection.type as connection_type,connection.id as connection_id,measure.score,base_measure.type,
                        ...
                        count(distinct metrics.id) as alerts, count(distinct issue.id) as issues, count(case when metrics.run_id=measure.last_run_id then metrics.id end) as recent_alerts_count, data.primary_columns,
                        case when schedules_mapping.measure_id is not null then true else false end as is_schedule,
                        case when base_measure.level = 'measure' and schedules_mapping.id is not null then max(schedules.next_run_at) end as next_run
                        from core.measure 
                        join core.base_measure on base_measure.id=measure.base_measure_id
                        join core.connection on connection.id=measure.connection_id
                        left join core.asset on asset.id=measure.asset_id
                        left join core.data on data.asset_id = asset.id
                        ...
                        left join core.metrics on metrics.measure_id=measure.id and measure.last_run_id = metrics.run_id and lower(metrics.drift_status) in ('high','medium','low')
                        left join core.issues as issue on issue.measure_id=measure.id and issue.is_delete=false and lower(issue.status) != 'resolved'
                        left join core.schedules_mapping on schedules_mapping.measure_id=measure.id and schedules_mapping.asset_id is null
                        left join core.schedules on schedules.id = schedules_mapping.schedule_id
                        ...
                        left join core.tags_mapping on tags_mapping.asset_id = attribute.id
                        left join core.terms_mapping on terms_mapping.asset_id = attribute.id
                        ...
                        left join core.tags on tags.id=tags_mapping.tags_id
                        left join core.terms on terms.id=terms_mapping.term_id
                        where measure.is_delete=false and connection.is_active=true
                        group by 
                        measure.id, base_measure.id, asset.id, connection.id, dimension.id, attribute.id,
                        attribute.name, data.primary_columns, schedules_mapping.id, schedules_mapping.measure_id, schedules.next_run_at
            )
            ...
            select 
                *,
                (
                        select 
                        json_agg(DISTINCT jsonb_build_object('id',domain.id,'name',domain.name)) 
                    from core.domain 
                    left join core.domain_mapping on domain_mapping.domain_id=domain.id
                    where domain_mapping.measure_id::text = asset_measures.id::text or domain_mapping.asset_id = asset_measures.asset_id
                    ) as domains
                
            from asset_measures
            where id='{measure_details}'
        ...
        with connection.cursor() as cursor:
            cursor = execute_query(connection, cursor, query_string)
            meassure_detail = fetchone(cursor)
            meassure_detail = meassure_detail.get('domains', [])
            if meassure_detail:
                for meassure_details in meassure_detail:
                    filter_domain_list.append(meassure_details.get('id', ''))
            if domains and len(domains) > 0 and filter_domain_list:
                custom_field_value_url = f"{channel_host_url}/integration/v2/custom_field/?allow_multiple=false&field_type=RICH_TEXT&limit=100&name_singular=DQLABS&skip=0"
                custom_field = call_api_request(
                    custom_field_value_url, 'get', '', token)
        ...
        log_error(
            f"set alation domain measure metrics ", e)
...
def get_alation_domains(channel, token):
    ...
    try:
        ...
        domains_list = call_api_request(api_url, 'get', '', token)
        ...
        if alation_domains:
            ...
            for domain in domains_list:
                ...
                if len(technical_names) > 1:
                    ...
                    filter_value = [domains_list_detail for domains_list_detail in domains_list_details if domains_list_detail.get('title') == parent_name]
                    if filter_value:
                    ...
                    domain.update({'technical_name': domain.get('title'), 'mapping_domain_id': domain.get('id')})
            connection = get_postgres_connection(config)
            ...
            with connection.cursor() as cursor:
                for domain in domains_list:
                    ...
                    for s_domain in domains:
                        if s_domain.get('id') == domain_id:
                            ...
                            check_existing_query = f"""
                                ...
                                and is_active = true and is_delete = false
                            """
                            print("check_existing_query1619", check_existing_query)
                            cursor = execute_query(
                                connection, cursor, check_existing_query)
                            existing_domain_data = fetchone(cursor)
                            if existing_domain_data:
                    ...
                    query_input = (
                        ...
                        domain_type,
                        level-1,
                        str(domain.get('mapping_domain_id')) if parent_domain_id else None,
                        True,
                        False,
                        'alation',
                        json.dumps(
...
def map_alation_domain_to_dqlabs(config: dict, channel: dict, table: dict):
    try:
        domain_input = []
        asset_id = config.get('asset_id')
...
def get_alation_tags(channel, token):
    ...
    try:
        ...
        tags = call_api_request(api_url, 'get', '', token)
...
def alation_tags_to_dQColumns_map(config: dict, tags: list, dq_column: dict):
    try:
        ...
        asset_id = dq_column.get('asset_id')
        connection = get_postgres_connection(config)
        with connection:
            with connection.cursor() as cursor:
                        ...
                        try:
                            ...
                            query_string = f"""
                                    insert into core.tags_mapping(
                                        id, level, tags_id, asset_id, attribute_id, created_date
                                    ) values {query_input}
                                """
                            cursor = execute_query(
                                connection, cursor, query_string)
...
# NEW CODE FOR DOMAIN STARTS FROM HERE
def get_domain_record(config, domain_id):
    domain = {}
    try:
        connection = get_postgres_connection(config)
        with connection.cursor() as cursor:
        ...
        log_error(
            f"Alation Connector - Get domain record ", e)
    finally:
        return domain


def get_domain_stats(config, domain_id):
    domain = {}
    try:
        """
        ...
        with connection.cursor() as cursor:
            ...
            query_string = f"""
               with recursive domain_hierarchy as (
                    ...
                    join core.asset on asset.id=domain_mapping.asset_id
                    ...
                    left join core.metrics as drift_alert on drift_alert.measure_id=measure.id
                    and drift_alert.run_id=measure.last_run_id and lower(drift_alert.status) in ('high','medium','low')
                    and (
                        drift_alert.attribute_id is null
                        or (drift_alert.attribute_id is not null and drift_alert.attribute_id=attribute.id)
                    )
                    left join core.issues on issues.measure_id=measure.id and issues.is_delete=false
                    and (
                    ...
                    where connection.is_active=true and asset.is_active=true
                    group by dom.root, asset.id, asset.score
                ), measure_domains as (
                    select dom.root as id,
                    (case when measure.allow_score=true then measure.score end) as score,
                    count(distinct drift_alert.id) as alerts,
                    count(distinct issues.id) as issues,
                    count(distinct measure.asset_id) as assets_count,
                    count(distinct measure.attribute_id) as attributes_count,
                    count(distinct measure.id) as measures_count,
                    null::uuid as assets, measure.id as measures,
                    array_remove(array_agg(distinct case when issues.id is not null then issues.id end), NULL) as issue_ids
                    from core.domain
                    join domain_list as dom on dom.id=domain.id
                    join core.domain_mapping on domain_mapping.domain_id=dom.id
                    join core.measure on measure.id=domain_mapping.measure_id
                    join core.base_measure on base_measure.id=measure.base_measure_id and base_measure.level='measure'
                    join core.connection on connection.id=measure.connection_id and connection.is_active=true
                    left join core.metrics as drift_alert  on drift_alert.measure_id=measure.id and drift_alert.run_id=measure.last_run_id and lower(drift_alert.status) in ('high','medium','low')
                    left join core.issues on issues.measure_id=measure.id and issues.is_delete=false
                    where measure.is_active=true
                    group by dom.root, measure.id
                ), issue_domains as (
                    with other_issues as (
                        select unnest(array_remove(string_to_array(string_agg(array_to_string(issue_ids ,','),','),','), '')) as id
                        from (
                            select issue_ids from asset_domains
                            union all
                            select issue_ids from measure_domains
                        ) as issues
                    )
                    select dom.root as id,
                    null::bigint as score, null::bigint as alerts,
                    count(distinct issues.id) as issues,
                    0 as assets_count,
                    0 as attributes_count,
                    0 as measures_count,
                    null::uuid as assets, null::uuid as measures,
                    null::uuid[] as issue_ids
                    from core.domain
                    join domain_list as dom on dom.id=domain.id
                    join core.domain_mapping on domain_mapping.domain_id=dom.id
                    join core.issues on (issues.asset_id=domain_mapping.asset_id or issues.measure_id=domain_mapping.measure_id)
                    ...
                    left join core.connection as connection on connection.id=measure.connection_id
                    left join core.attribute as attribute on attribute.id=measure.attribute_id and attribute.is_selected=true
                    where issues.is_delete=false
                    and (issues.asset_id is null or (asset.id is not null and asset.is_active = true))
                    and (
                        issues.attribute_id is null
                        or (issues.attribute_id is not null and issues.attribute_id=attribute.id)
                    )
                    and issues.id::text not in (select id from other_issues)
                    group by dom.root
                ), grouped_domains as (
                    select * from asset_domains
                    union all
                    select * from measure_domains
                    union all
                    select * from issue_domains
                ), domain_stats as (
                    select id,
                    avg(score) as score,
            ...
            if domain:
                domain = next(
                    (
                        stat
                        for stat in domain
                        if str(stat.get("id")) == str(domain_id)
                    ),
                    {}
                )
            else:
        ...
        log_error(
            f"Alation Connector: Get domain Stats ", str(e))
    finally:
        return domain


def get_domain_asset_stats(config, domain_id):
    asset_stats = {}
    try:
        """
        Get domain details by domain id
        ...
        with connection.cursor() as cursor:
            ...
            query_string = f"""
                select 
                    coalesce(sum(case when lower(type) in ('base table', 'query', 'table', 'external table') then 1 else 0 end), 0) as tables,
                    coalesce(sum(case when lower(type) in ('view') then 1 else 0 end), 0) as views,
                    coalesce(sum(case when lower(type) in ('pipeline') then 1 else 0 end), 0) as pipeline,
                    coalesce(sum(case when lower(type) in ('reports', 'sheet', 'dashboard') then 1 else 0 end), 0) as reports
                    from (
                        select distinct asset.id, asset.type
                        from core.asset
                        join core.connection on connection.id=asset.connection_id                       
                        join core.domain_mapping on domain_mapping.asset_id = asset.id
                        ...
                        and connection.is_active=true and asset.is_active = true and asset.is_delete = false
                ) as temp
            """
            cursor = execute_query(connection, cursor, query_string)
            asset_stats = fetchone(cursor)
            asset_stats = asset_stats if asset_stats else {}
        return asset_stats
    except Exception as e:
        log_error(
            f"Alation Connector: Get domain Asset Stats ", str(e))
    finally:
        return asset_stats


def get_domain_measure_count(config, domain_id):
    measures = 0
    try:
        """
        Get domain details by domain id
        ...
        with connection.cursor() as cursor:
            ...
            query_string = f"""
                ...
                and (
                    (measure.attribute_id is null and attribute.id is null) 
                    or (measure.attribute_id is not null and attribute.is_selected = true)
                )
            """
            cursor = execute_query(connection, cursor, query_string)
        ...
        log_error(
            f"Alation Connector: Get domain measures count ", str(e))
    finally:
        return measures


def get_domain_attributes_count(config, domain_id):
    attributes = 0
    try:
        """
        Get domain details by domain id
        ...
        with connection.cursor() as cursor:
            ...
            query_string = f"""
                select 
                    count(distinct attribute.id) as count
                from core.asset
                join core.connection on connection.id=asset.connection_id
            ...
            attributes = attributes.get('count', 0) if attributes else 0
        return attributes
    except Exception as e:
        log_error(
            f"Alation Connector: Get domain attributes count ", str(e))
    finally:
        return attributes
...
# NEW CODE FOR MEASURES STARTS FROM HERE
