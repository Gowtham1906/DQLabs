...
from dqlabs.app_constants.dq_constants import CUSTOM, BUSINESS_RULES, SEMANTICS
from dqlabs.tasks import create_tasks

# Celery queue name for the dedicated semantic-worker pods.
# All other categories route to the default queue.
...
def create_dag(dag_info: dict, category: str) -> DAG:
    ...
    # Route SEMANTICS tasks to the dedicated semantic-worker Celery queue.
    # Semantic workers run the ML/NLP stack (PyTorch, SpaCy, transformers).
    # All other categories run on the standard base worker queue.
    dag_default_args = {**default_args}
    if category == SEMANTICS:
        dag_default_args["queue"] = SEMANTIC_QUEUE
